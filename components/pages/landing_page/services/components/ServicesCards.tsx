    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 w-full" style={{ perspective: '1200px' }}>
      {services.map((service, idx) => (
        <TiltCard
          ...
          contentZ={20}
          className="service-card group rounded-xl p-6 lg:p-8 flex flex-col bg-[var(--color-surface-card)] backdrop-blur-sm border border-[var(--color-border)] hover:border-[var(--color-accent)]/25 transition-colors duration-300"
        >
          {/* Icon Box */}
